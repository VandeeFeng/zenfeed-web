<script lang="ts">
    ...
    const GROUP_BY_LABEL_STORAGE_KEY = "zenfeed_past24h_group_by_label";
    ...
    function updateAvailableLabels(feeds: FeedVO[]) {
        const allLabels = new Set<string>([DEFAULT_GROUP_BY_LABEL, 'pub_time']);
        const labelValueCounts: { [key: string]: { [value: string]: number } } = {};

        feeds.forEach((feed) => {
            if (feed.time) {
                const dateStr = feed.time.split('T')[0];
                if (!labelValueCounts['pub_time']) labelValueCounts['pub_time'] = {};
                labelValueCounts['pub_time'][dateStr] = (labelValueCounts['pub_time'][dateStr] || 0) + 1;
            }
        ...
        const usefulLabels = Array.from(allLabels).filter((labelKey) => {
            if (labelKey === DEFAULT_GROUP_BY_LABEL || labelKey === 'pub_time') return true;
